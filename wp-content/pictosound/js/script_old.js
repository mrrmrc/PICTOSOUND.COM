...
// AGGIUNGI QUESTA NUOVA FUNZIONE
    function updateDurationOptionsUI() {
        if (typeof pictosound_vars === 'undefined') {
            console.warn("WARN: pictosound_vars non è definito. Impossibile aggiornare le opzioni di durata.");
            return;
        }
        ...
        durationRadios.forEach(radio => {
            ...
            const cost = pictosound_vars.duration_costs[durationValue];
            const label = document.querySelector(`label[for="${radio.id}"]`);
            let reason = '';
            let enableOption = true;

            if (cost > 0) { // Se la durata ha un costo
            ...
            if (label) {
                ...
                if (creditSpan) {
                    // Mostra il costo o la ragione della disabilitazione
                    if (!enableOption && cost > 0) {
                        creditSpan.textContent = reason.split('(')[0].trim(); // Es. "Login richiesto"
                    } else {
                         // Ripristina il testo originale del costo/free
                        if (radio.id === 'duration40') creditSpan.textContent = 'FREE';
                        else if (radio.id === 'duration60') creditSpan.textContent = '1 cr.';
                        else if (radio.id === 'duration120') creditSpan.textContent = '2 cr.';
                        else if (radio.id === 'duration180') creditSpan.textContent = '3 cr.';
                        else if (radio.id === 'duration240') creditSpan.textContent = '4 cr.';
                        else if (radio.id === 'duration360') creditSpan.textContent = '5 cr.';
                        // Potresti voler salvare i testi originali in data-attributes per un ripristino più pulito
                    }
                }
                label.title = enableOption ? reason : reason; // Tooltip
                label.style.opacity = enableOption ? '1' : '0.5';
                if (!enableOption) {
                    label.classList.add('disabled-option-custom'); // Aggiungi una classe per stilizzazione CSS se vuoi
                } else {
        ...
        // Se l'opzione attualmente selezionata è disabilitata, e c'è un'opzione gratuita, seleziona quella
        ...
        if (currentlySelectedRadio && currentlySelectedRadio.disabled) {
            const freeOption = document.getElementById('duration40'); // Assumendo che duration40 sia sempre la gratuita
            if (freeOption && !freeOption.disabled) {
                freeOption.checked = true;
            }
        }
        console.log("LOG: UI opzioni durata aggiornata. Crediti utente:", pictosound_vars.user_credits);
    }
    ...
    domElements.generateMusicButton.addEventListener('click', async () => {
        ...
        if (!stableAudioPromptForMusic || stableAudioPromptForMusic.toLowerCase().includes("errore") || stableAudioPromptForMusic.toLowerCase().includes("data analysis not available")) {
            ...
            if (!stableAudioPromptForMusic || stableAudioPromptForMusic.toLowerCase().includes("errore") || stableAudioPromptForMusic.toLowerCase().includes("data analysis not available")) {
                ...
                updateProgressMessage("Errore nella preparazione del prompt. Riprova o seleziona un'immagine.", false);
                return;
            }
        }
        ...
        try {
            ...
            const response = await fetch('/wp-content/pictosound/generate_music.php', {
                method: 'POST',
            ...
            if (response.ok && contentType && contentType.indexOf("application/json") !== -1) {
                ...
                setTimeout(() => {
                    if (domElements.statusDiv.textContent === "Musica generata (formato diretto)!")
                        domElements.statusDiv.style.display = 'none';
                }, 3000);

                const audioBlob = await response.blob();
                const audioUrlForPlayer = URL.createObjectURL(audioBlob);
                domElements.audioPlayer.src = audioUrlForPlayer;
                domElements.audioInfo.textContent = `Traccia audio generata.`;
                domElements.downloadAudioLink.href = audioUrlForPlayer;
                domElements.downloadAudioLink.download = `pictosound_audio_${Date.now()}.mp3`;
                domElements.downloadAudioLink.style.display = 'inline-flex';
                domElements.audioPlayerContainer.style.display = 'block';
                domElements.downloadCompositeImageLink.style.display = 'none';
                domElements.downloadQrOnlyLink.style.display = 'none';
